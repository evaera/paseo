...
The `browser_list_import_sources` and `browser_import_browser_data` agent tools, plus `paseo browser import-sources` and `paseo browser import`, use the same path. Paseo desktop shows a native confirmation before reading browser data. The confirmation displays the complete domain allowlist. Imports fail closed when that displayed list exceeds 1,000 characters. Import responses contain imported, queued, and skipped counts plus warnings, never cookie or storage values. Cancelling stops remaining work but does not roll back cookies or storage records already applied.
