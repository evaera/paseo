...
The desktop app applies **Service URLs: Where agent-run and other running scripts open HTTP(S) URLs**: `in-app` opens a focused browser tab in the workspace, `external` uses the system browser, and `ask` offers Paseo, the external browser, or Cancel. Cancel and window dismissal open and persist nothing. The app setting is the only persisted policy. Choosing `external` explicitly authorizes running scripts and agents to open these service URLs in the system browser. `ask` fails closed when the desktop cannot show its choice dialog. Requests are queued per workspace; a timed-out dialog may remain visible, but its eventual answer cannot open a URL or persist a setting.
