...
On macOS, agents and terminals started in a workspace receive `BROWSER` and `PATH` entries for Paseo's packaged `open` wrapper. Plain HTTP(S) links opened by tools such as Plannotator are dispatched to `paseo browser open-service-url`. Once one compatible desktop accepts the dispatch, the wrapper exits without waiting for a human choice. Unavailable or ambiguous hosts and short dispatch failures use `/usr/bin/open`; an accepted, pending `ask` never triggers that fallback.

The desktop app applies **Service URLs: Where assistant links, agents, and running scripts open HTTP(S) URLs**. Assistant-chat links and routed process opens use the same policy: `in-app` opens a focused browser tab in the workspace, `external` uses the system browser, and `ask` offers Paseo, the external browser, or Cancel. Cancel and window dismissal open and persist nothing. The app setting is the only persisted policy. Choosing `external` explicitly authorizes these links to open in the system browser. `ask` fails closed when the desktop cannot show its choice dialog. Requests are queued per workspace; a timed-out dialog may remain visible, but its eventual answer cannot open a URL or persist a setting.

Set `PASEO_BROWSER_OPEN_EXTERNAL=1` for a process that must always use the system browser, or run `paseo browser open --external <http(s)-url>`. `paseo browser open <url>` remains the explicit in-app operation and uses `PASEO_WORKSPACE_ID` when `--workspace` is omitted. An explicitly configured `BROWSER` is preserved; set `PASEO_BROWSER_OPEN_OVERRIDE=1` to replace it with Paseo's wrapper. Explicit `open` flags, non-HTTP schemes, and processes without `PASEO_WORKSPACE_ID` still pass through unchanged.

MCP keeps `browser_new_tab` as its raw in-app operation. Raw external-open MCP remains unavailable because it would bypass user policy with a user-visible host action. `browser_open_service_url` differs: it waits for the existing desktop policy, and `external` runs only when the user selected or persisted that setting. Direct CLI can add `--wait` to receive the final `dismissed`, `in-app`, or `external` disposition instead of returning after accepted dispatch. Waited choices time out after five minutes.
