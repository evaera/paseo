...
- `paseo layout inspect/open/split/move-tab/move-pane`
...
>
> **In-app browser profiles.** Browser tabs persist a desktop-owned profile id. Default keeps the original `persist:paseo-browser` Electron session, while each named profile receives a separate persistent partition and therefore a separate cookie jar, authentication cache, and site storage. New profiles start empty, and Default cannot be deleted. Main owns the only deletion boundary: every renderer, MCP, and CLI request reaches the same IPC handler and native confirmation. An accepted deletion broadcasts before Electron clears storage, cache, and authentication data so every renderer can destroy live guests, then broadcasts the metadata change so windows refresh and prune stale profile ids. The partition directory remains Electron-owned instead of being recursively removed while live handles may exist on Windows. After every `did-attach`, the renderer registers its browser id, workspace id, profile id, and guest `WebContents` id. Main accepts the registration only when the profile id exists and the guest belongs to both the calling renderer and the partition main derives from that id. Registration repeats because reparenting a retained `<webview>` can replace its guest without replacing the DOM element.
>
> **In-app browser window opens.** Ordinary link opens, including Shift-clicked links, become Paseo workspace tabs in the source tab's profile. Script-created opens with popup features or a named window target and POST-backed opens remain secured Electron child windows in the source session, preserving profile isolation, `window.opener`, `postMessage`, named-window reuse, request bodies, and `window.close()` for OAuth, payment, and similar popup protocols. Unsupported URL schemes are denied before either path.
>
...
>
> **Workspace layout control.** Pane trees and browser tabs are client-local. The daemon brokers `workspace.layout.execute` requests from the CLI and transport-neutral agent tools to a connected app layout host. Inspection returns stable workspace, pane, and tab IDs; mutations use those IDs rather than daemon layout policy. Tab placement can name an existing pane or create a left, right, top, or bottom split.
>
